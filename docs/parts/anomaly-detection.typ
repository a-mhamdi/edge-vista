...
#title-slide[Three broad families of methods][Statistical, Machine Learning, and Signal Processing]
...
/ Robust z-score: median absolute deviation (MAD) in place of mean/std for outlier-resistant scoring
...
Any periodic _(or integrable)_ signal $x(t)$ can be decomposed into a sum of sinusoids, each characterised by a frequency $f$, an amplitude, and a phase.
...
#block(
  ...
  inset: 7pt,
  width: 100%,
  text[⚡ This decomposition is the theoretical backbone behind the FFT/STFT detectors above — in practice $X(f)$ must be estimated from *sampled*, *finite-length* data via the DFT.],
)
...
#info[In practice sensors provide a finite, uniformly sampled sequence rather than a continuous signal — the DFT is the CFT evaluated at $N$ discrete frequency bins for such a sequence, and is the object actually computed on embedded hardware.]
...
#grid(
  ...
  [
    #align(center)[
      #image("../assets/dft-hm.png", width: 80%)
    ]
  ],
)

#url-block("codes/anomaly-detection/dft-hm.py")
...
#grid(
  ...
  [
    #align(center)[
      #image("../assets/butterfly.svg", width: 50%)
    ]
  ],
)
---
#align(center)[
  #image("../assets/fft.svg", width: 60%)
]
...
#grid(
  ...
  [
    #align(center)[
      #image("../assets/anomaly-detection-fft.png", height: 80%)
    ]
  ],
)

#url-block("codes/anomaly-detection/anomaly-detection-fft.py")

---

=== Power Spectral Density (PSD)

#def[The PSD describes how a signal's power is distributed across frequency. For a wide-sense-stationary process, the Wiener–Khinchin theorem gives it as the Fourier transform of the autocorrelation function $R_x (tau)$.]
...
#block(
  ...
  [
    ...
    #v(4pt)
    Equivalently, via the periodogram limit:
    $ S_(x x)(f) = lim_(T -> infinity) frac(1, T) lr(| X_T (f) |)^2 $ <eq:wiener-khinchin>
  ],
)
#v(6pt)
#text(weight: "bold", fill: accent)[Total power (Parseval)]
$
...
#grid(
  ...
  gutter: 14pt,
  [
    #text(weight: "bold", fill: accent)[Estimation methods]
    #table(
      columns: (auto, auto),
      stroke: 0.4pt + muted,
      inset: 5pt,
  ...
  [
    #align(center)[
      #image("../assets/anomaly-detection-psd.png", height: 80%)
    ]
  ],
)

#url-block("codes/anomaly-detection/anomaly-detection-psd.py")
