...
\begin{tikzpicture}[x=.8cm,y=.8cm]
    ...
    \node[left,color=blue!50!green] at ($(b1)-(0.25,1)$){\tiny\it (Model evaluation)};
    \node (b2) at (-7.3643,-0.397) {};
    \draw[-latex,out=180,in=270,color=blue!50!green,thick](b2)to(b1);

    \node[below] (b0) at (-6,0.9) {\tiny\it (Feature selection, scaling, ...)};

    \node (b7) at (-2.7682,2.4147) {};
    \node (b8) at (-2.4158,1.043) {};

    \node[color=red!50!blue] (b91) at (-1.008,2.2225) {\tiny{\bfseries Refine model}};
    \node[color=red!50!blue] (b92) at (-0.608,1.7225) {\tiny{\it(Hyperparameters tuning, ...)}};
